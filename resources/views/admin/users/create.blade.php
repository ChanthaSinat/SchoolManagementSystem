...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ __('Add :role', ['role' => $roleLabel]) }}</h1>
            </div>
            <p class="text-slate-500 font-medium ml-12">Create a new account manually in the system.</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="px-3 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-black rounded-full border border-indigo-100 uppercase tracking-wider">New {{ $roleLabel }}</span>
        </div>
    </div>
    ...
    <div class="bg-white/60 backdrop-blur-xl rounded-3xl border border-white shadow-xl shadow-slate-200/50 overflow-hidden">
        <form method="POST" action="{{ route('admin.users.store') }}" class="divide-y divide-slate-100">
            @csrf
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <x-text-input id="first_name" name="first_name" type="text" class="w-full px-4 py-3 bg-white/50 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all" :value="old('first_name')" required autofocus />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="last_name" name="last_name" type="text" class="w-full px-4 py-3 bg-white/50 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all" :value="old('last_name')" required />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="email" name="email" type="email" class="w-full px-4 py-3 bg-white/50 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all" :value="old('email')" required />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="phone" name="phone" type="text" class="w-full px-4 py-3 bg-white/50 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all" :value="old('phone')" />
            ...
            <div class="p-8 space-y-6 bg-slate-50/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <x-input-label for="password" :value="__('Password')" class="text-xs font-black uppercase tracking-wider text-slate-500 ml-1" />
                        <x-text-input id="password" name="password" type="password" class="w-full px-4 py-3 bg-white/50 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all" required autocomplete="new-password" />
                        <x-input-error class="mt-2" :messages="$errors->get('password')" />
                    </div>

                    <div class="space-y-2">
                        <x-input-label for="password_confirmation" :value="__('Confirm Password')" class="text-xs font-black uppercase tracking-wider text-slate-500 ml-1" />
                        <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="w-full px-4 py-3 bg-white/50 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all" required autocomplete="new-password" />
            ...
            <div class="p-8 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center sm:text-right">
                    You can assign classes/sections<br>after initial account creation
                </p>
            </div>
        </form>
    </div>
