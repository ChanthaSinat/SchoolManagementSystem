...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900">Edit Semester / Term</h1>
            <p class="text-slate-500 text-sm font-medium mt-1">{{ $semester->name }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.semesters.update', $semester) }}" class="space-y-6">
            ...
            <div>
                ...
                <select id="academic_year_id" name="academic_year_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 text-sm focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 outline-none">
                    ...
                    @foreach ($academicYears as $year)
                        <option value="{{ $year->id }}" @selected(old('academic_year_id', $semester->academic_year_id) == $year->id)>{{ $year->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $semester->name) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 outline-none text-sm">
                ...
                <div>
                    ...
                    <input id="code" name="code" type="text" value="{{ old('code', $semester->code) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 outline-none text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="order" name="order" type="number" min="1" value="{{ old('order', $semester->order) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 outline-none text-sm">
                ...
                <div class="flex items-center gap-2 mt-6 md:mt-8">
                    <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" {{ old('is_active', $semester->is_active) ? 'checked' : '' }}>
