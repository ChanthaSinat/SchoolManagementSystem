...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900">Add School Type</h1>
            <p class="text-slate-500 text-sm font-medium mt-1">Define a new school structure and year levels.</p>
        </div>
    ...
    <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-slate-100 shadow-sm p-8">
        <form method="POST" action="{{ route('admin.school-types.store') }}" class="space-y-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="name" class="block text-xs font-black text-slate-600 uppercase tracking-widest mb-1">Name</label>
                    <input id="name" name="name" type="text" value="{{ old('name') }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 outline-none text-sm">
                ...
                <div>
                    ...
                    <input id="code" name="code" type="text" value="{{ old('code') }}" required placeholder="e.g. high_school" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 outline-none text-sm">
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-500 mb-2">Add labels like “Grade 1”, “Grade 2”, “Year 1”, “Year 2”, etc.</p>
                <div id="year-levels-container" class="space-y-2">
                    @php $oldLevels = old('year_levels', []); @endphp
                    @forelse ($oldLevels as $level)
            ...
            <div class="flex items-center gap-2">
                <input id="is_default" name="is_default" type="checkbox" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" {{ old('is_default') ? 'checked' : '' }}>
